...
public class Stream_07_Test {
    ...
    Function<List<Pizza>, List<Pizza>> filterPizza = l -> l.stream().filter(p -> p.getPrice() >= 1000).collect(Collectors.toList());

    // TODO créer une fonction List<Pizza> -> List<Pizza>
    // TODO seules les pizzas ayant un prix >= 1000 sont conservées
    // TODO .parallel()
    Function<List<Pizza>, List<Pizza>> parallelFilterPizza = l -> l.stream().parallel().filter(p -> p.getPrice() >= 1000).collect(Collectors.toList());
